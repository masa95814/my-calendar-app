...
- **予算アラート（Slack）**: 請求額が月の予算の 50% / 90% / 100% を超えたら Slack に通知する（100% はメンション付き。請求先の管理者へのメールも既定どおり届く）。予算 → Pub/Sub → Cloud Run の `/webhooks/budget`（push の ID トークンを検証）→ Slack。予算のメッセージは 1 日に何度も届くため、月ごとにまだ知らせていないしきい値だけを知らせる（Firestore の `budgetAlerts`）
