...
- アカウントに呼び名（例: メイン、ギブリー）を付けられる。画面ではメールアドレスの代わりに呼び名を出す。未設定なら Workspace のドメインかメールアドレスの @ の前（2026-09-24 追加）
...
| 同期範囲     | 今日（送信元のタイムゾーンでの 0:00）から何日先まで同期するか（30 / 60 / 90 日。既定 60 日）。今日すでに終わった予定も同期する |
| フィルタ条件 | F3 参照                                                                                                                        |
| 出力設定     | F4 参照                                                                                                                        |
...
| 長さの上限                 | 元予定がこれより長いときは、ミラーの終了を「開始 + 上限」にする（例: 1 時間で入っているが実際は 30 分で終わる定例）。終日の予定には使わない（2026-09-24 追加）                                                                                                                 | 上限なし                                  |
| 上限をかける予定           | 長さの上限を、タイトルにいずれかのキーワードを含む予定だけに使う。空ならすべての予定（2026-09-24 追加。例: 「農活」の定例だけ 30 分にし、「SalesforceMTG」は 1 時間のまま）                                                                                                    | すべての予定                              |
| 種別を切り替えるキーワード | タイトルにいずれかを含む予定は、もう一方の種別（予定あり ⇔ 不在）で作り、件名もその種別用にできる（空欄なら「不在」「予定あり」）。1 つの同期設定の中で種別を使い分ける。同期先が個人なら使えない（2026-09-25 追加。例: 種別は予定あり、「外出」を含む予定だけ不在「外出中」） | なし                                      |
...
- 同期範囲から外れた過去のミラー予定はそのまま残す（履歴として扱う）。対応表に元予定の終了時刻を持ち、範囲の始まりより前に終わった予定のミラーは、全件同期で元予定が見えなくても掃除しない。元予定がキャンセルされた場合だけ消す（2026-09-24 修正。以前は毎晩の全件同期で消えていた）
...
- 同期設定がエラーになったとき・復旧したとき、アカウントの連携が切れた（再認証が必要）ときに Slack へ通知する（状態が変わったときだけ。2026-09-25 追加）
...
1. 送信元カレンダーごとに `events.list` を呼ぶ。`syncToken` があれば差分、なければ `timeMin=今日の 0:00` `timeMax=今+windowDays` で全件（`singleEvents=true`）
2. 取得した各予定について
...
3. 新しい `syncToken` を保存する。410 Gone なら `syncToken` を破棄して全件同期に戻り、対応表にあるのに元予定が無いミラー予定を削除する（孤児の掃除。範囲の始まりより前に終わった予定のミラーは残す）
...
| PATCH                     | `/api/accounts/:id`                 | 呼び名の変更（`{ label }`。空か null で未設定に戻す） |
...
| アカウント | 連携アカウント一覧、追加、再認証、解除、呼び名の変更                                                                                                                                                                                                                          |
| 同期設定   | 一覧は同期元のアカウントを選び、ほかの全アカウントを同期先として並べる（設定済みは緑の点、未設定は灰色で「追加」。逆方向の有無も各行に出す。2026-09-24 変更）。方向ごとに有効 / 無効と設定を個別に編集できる。編集フォームは F2〜F4。同期先が個人なら種別を「予定あり」に固定 |
| 設定       | ログアウト、バージョン情報                                                                                                                                                                                                                                                    |
