...
1 つの同期設定 = 1 方向。「A → B」と「B → A」の両方が必要なら 2 つ作る（UI に「逆方向も同時に作る」ショートカットを用意する）。
...
| 除外キーワード   | タイトルまたは説明にいずれかを含む予定は除外する                      | なし     |
...
| 同期設定   | 一覧と編集フォーム（F2〜F4）。同期先が個人なら種別を「予定あり」に固定 |
| 設定       | ログアウト、バージョン情報                                             |
